...
export const MeSection: React.FC = () => {
  ...
  const sampleDialogue = t("me.sampleDialogue");
    ...
    <div ref={containerRef} className="container mx-auto max-w-5xl px-4 sm:px-6 py-6 space-y-8 select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="me-card-column md:col-span-6 space-y-6">
          {/* Your Profile Card */}
          <Card className="rounded-3xl p-6 border border-emerald-500/20 bg-card/50 dark:bg-card/40 shadow-lg shadow-emerald-500/[0.03] backdrop-blur-xl backdrop-saturate-150">
          ...
          <Card className="rounded-3xl p-6 border border-emerald-500/20 bg-card/50 dark:bg-card/40 shadow-lg shadow-emerald-500/[0.03] backdrop-blur-xl backdrop-saturate-150">
            ...
            <CardContent className="p-0 space-y-5 text-xs">
              ...
              >
                {savedSuccess ? (
                  <>
                    <Check className="size-4 text-emerald-300 animate-pop-in" />
                    <span>{t("me.savedAll")}</span>
                  </>
                ) : (
                  <>
                    <Save className="size-4" />
                    <span>{t("me.saveAll")}</span>
                  </>
                )}
              </Button>
            </CardContent>
        ...
        <div className="me-card-column md:col-span-6 space-y-6">
          <Card className="rounded-3xl p-6 border border-emerald-500/20 bg-card/50 dark:bg-card/40 shadow-lg shadow-emerald-500/[0.03] backdrop-blur-xl backdrop-saturate-150 flex flex-col h-full">
            ...
            <CardContent className="p-0 space-y-4 flex-1 flex flex-col text-xs">
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="font-semibold text-foreground/80">{t("me.transcriptLabel")}</span>
                  <button
                    onClick={() => setTranscript(sampleDialogue)}
                    className="text-[11px] text-primary hover:underline font-light cursor-pointer"
                  >
                    {t("me.loadSample")}
                  </button>
                </div>
                <textarea
                  ...
                  placeholder={t("me.transcriptPlaceholder")}
                  rows={6}
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  ...
                  <span>{isAnalyzing ? t("me.auditing") : t("me.auditBtn")}</span>
                </Button>
                ...
                {transcript && (
                  <button
                    ...
                    className="text-[11px] text-muted-foreground hover:text-foreground font-light cursor-pointer"
                  >
                    {t("me.clearText")}
                  </button>
                )}
              ...
              {isAnalyzing && (
                <div className="p-6 rounded-2xl bg-muted/30 border border-border flex flex-col items-center justify-center gap-3 text-center my-2 animate-in fade-in duration-300">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-semibold text-foreground">{t("me.auditLoadingTitle")}</p>
                    <p className="text-[11px] text-muted-foreground font-lato-light-italic">
                      {t("me.auditLoadingDesc")}
                    </p>
                  </div>
                </div>
              )}
